...
def test_you_yall():
    ...
    tests = [("you", "y'all"), ("You", "Y'all"), ("your", "y'all's"), ("Your", "Y'all's")]
    for given, expected in tests:
        out = getoutput(f'{prg} {given}')
        assert out.strip() == expected.strip()
...
def test_think():
    ...
    out = getoutput(f"{prg} think")
    assert out.strip() == "reckon"

    out = getoutput(f"{prg} thinking")
    assert our.strip() == "reckonin'"
...
def test_fixin():
    ...
    out = getoutput(f"{prg} preparing")
    assert out.strip() == "fixin'"

    out = getoutput(f"{prg} 'getting ready'")
    assert out.strip() == "fixin'"
